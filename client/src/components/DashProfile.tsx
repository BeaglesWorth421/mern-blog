...
export default function DashProfile() {
  ...
  const uploadImage = async () => {
    ...
    uploadTask.on(
      ...
      (snapshot) => {
        ...
        setImageFileUploadProgress(parseFloat(progress.toFixed(0)));
      },
      (error) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!currentUser || !currentUser._id) {
      // Handle the case where currentUser is not defined or _id is missing
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div className="relative w-32 h-32 self-center cursor-pointer shadow-md overflow-hidden rounded-full" onClick={() => filePickerRef.current?.click()}>
          ...
          <img
            ...
            alt="user"
            className={`rounded-full w-full h-full object-cover border-8 border-[lightgray] ${imageFileUploadProgress !== null && imageFileUploadProgress < 100 ? "opacity-60" : ""}`}
          />
        </div>
